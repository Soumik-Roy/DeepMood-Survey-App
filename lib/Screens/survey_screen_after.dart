...
class _SurveyPage2State extends State<SurveyPage2> {
  ...
  final TextEditingController _recentApplication = TextEditingController();
  ...
  updatePostTime(String newPosttime) async{
    SharedPreferences savedData = await SharedPreferences.getInstance();
    await savedData.setString('posttime', newPosttime);
  }
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: <Widget>[
                  ...
                  Center(
                    child: Text("Post Trial", style: GoogleFonts.manrope(fontSize: 23),),
                  ),
                  const Divider(
                    color: Colors.teal,
                    thickness: 2,
                  ...
                  const Text("The recent Application you used :", style: TextStyle(fontWeight: FontWeight.w500),),
                  Padding(
                    padding: const EdgeInsets.symmetric(horizontal: 35.0, vertical: 15),
                    child: TextField(
                      controller: _recentApplication,
                      autofocus: true,
                    ),
                  ),
                  ...
                  TextButton(
                    onPressed: () async{
                      ...
                      DateTime dt2 = dt1.add(const Duration(minutes: 40));
                      DateTime dt3 = DateTime.parse(dt1.toString());
                      print("$dt1 , $dt3 ,  $dt2 , ${dt2.isAfter(dt1)}");
                      if (buttonActive) {
  ...
  /// SUBMIT AND SEND TO FIREBASE
  Future<void> submitForm() async{
    await Firebase.initializeApp();
    final db2 = FirebaseFirestore.instance;
    List<String> dateTime = DateTime.now().toString().split(" ");
    updatePostTime(dateTime[0]+' , '+dateTime[1].substring(0, 5));
    await db2.collection('PostTrial_Survey').add(
        {
          'name': userName,
          'date' : dateTime[0],
          'time' : dateTime[1].substring(0, 8),
          'Recent Application' : _recentApplication.text,
